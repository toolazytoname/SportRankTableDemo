...
@interface EasyTableView ()<UIGestureRecognizerDelegate>
@property (nonatomic, strong) UIPanGestureRecognizer *gestureRecogizer;
...
- (void)createTableWithOrientation:(EasyTableViewOrientation)orientation {
    ...
    self.gestureRecogizer = [[UIPanGestureRecognizer alloc]initWithTarget:self
                                                                   action:@selector(panGestureRecognizerAction:)];
    self.gestureRecogizer.delegate = self;
    
    [tableView addGestureRecognizer:self.gestureRecogizer];
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    BOOL isScrollToEnd = (self.tableView.frame.size.width + self.tableView.contentOffset.y) >= self.tableView.contentSize.height;
    if ((self.tableView.contentOffset.y == 0) && point.x > 0)
    {
        self.tableView.panGestureRecognizer.enabled = NO;
        return YES;
    }else if (self.tableView.contentOffset.y == 0 && point.x < 0) {
        self.tableView.panGestureRecognizer.enabled = YES;
        return NO;
    }else if (isScrollToEnd && point.x < 0) {
        self.tableView.panGestureRecognizer.enabled = NO;
        return YES;
    }else{
        self.tableView.panGestureRecognizer.enabled = YES;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([delegate respondsToSelector:@selector(easyTableView:heightOrWidthForCellAtIndexPath:)]) {
        _cellWidthOrHeight = [delegate easyTableView:self heightOrWidthForCellAtIndexPath:indexPath];
    }
